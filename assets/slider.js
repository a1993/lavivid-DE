...
class CarouselSlider extends HTMLElement {
  ...
  initSlider() {
    ...
    // Only enable the grabbable cursor / drag listeners on pointer: fine
    // and when this slider hasn't explicitly disabled dragging via
    // `data-disable-drag="true"` on the `<carousel-slider>` element.
    if (window.matchMedia('(pointer: fine)').matches && this.dataset.disableDrag !== 'true') {
  ...
  addListeners() {
    ...
    // Attach mouse drag listeners only for fine pointers and when drag
    // hasn't been disabled via `data-disable-drag="true"`.
    if (window.matchMedia('(pointer: fine)').matches && this.dataset.disableDrag !== 'true') {
