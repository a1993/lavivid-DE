...
if (!customElements.get("variant-picker")) {
  class VariantPicker extends HTMLElement {
    ...
    handleVariantChange(evt) {
      ...
      const productUrl =
        evt.target.dataset.productUrl || evt.detail?.productUrl;
      if (productUrl) {
    ...
    updateAvailability() {
      ...
      this.optionSelectors.forEach((selector, selectorIndex) => {
        ...
        if (
          this.dataset.availability === "prune" &&
          optionThatExists &&
          selector.querySelector(
            '.js-option.is-nonexistent:is(:checked, [aria-selected="true"])'
          )
        ) {
          setTimeout(() => {
    ...
    static updateLabelText(evt) {
      ...
      // 从 input 的 value 属性获取完整值，而不是从 .js-value 获取 split 后的值
      colorText.textContent = evt.target.value;
    }
    ...
    updatePrice() {
      ...
      if (variant) {
        ...
        if (variant.compare_at_price > variant.price) {
          priceCurrentEl.querySelector(".js-label").textContent =
            theme.strings.salePrice;
          priceCurrentEl.querySelector(".js-value").innerHTML =
            this.data.formatted[variant.id].price;
    ...
    updateSku() {
      ...
      this.sku.textContent = skuAvailable ? this.variant?.sku?.slice(0, 6) : "";
  ...
  const preStyleServiceElement = document.querySelector('.option-selector[data-option="PRE - STYLE SERVICE"]');
  if (preStyleServiceElement) {
      preStyleServiceElement.style.display = 'none';
  }
}
