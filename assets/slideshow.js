...
if (!customElements.get('slide-show')) {
  class SlideShow extends HTMLElement {
    ...
    /**
     * Saves initial 'aria-hidden' and 'tabindex' of links in slides.
     */
    saveSlideVisibility() {
      this.slides.forEach((slide) => {
        slide.querySelectorAll('a[tabindex]').forEach((link) => {
          link.dataset.dataSlideshowPriorTabindex = link.getAttribute('tabindex');
        });
      });
    }
    ...
    setSlideVisibility() {
      this.slides.forEach((el, index) => {
        ...
        if (index === this.currentIndex) {
          ...
          links.forEach((link) => {
            if (link.dataset.dataSlideshowPriorTabindex !== undefined) {
              link.setAttribute('tabindex', link.dataset.dataSlideshowPriorTabindex);
            } else {
